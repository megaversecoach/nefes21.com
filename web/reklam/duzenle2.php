...
$sorgu = $baglanti->query("SELECT * FROM yatay WHERE id =".(int)$_GET['id']); //id değeri ile düzenlenecek verileri veritabanından alacak sorgu
...
if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
	...
	if ($baslik<>"" && $icerik<>"") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
		...
		if ($baglanti->query("UPDATE yatay SET baslik = '$baslik', icerik = '$icerik' WHERE id =".$_GET['id'])) // Veri güncelleme sorgumuzu yazıyoruz.
		{
			header("location:duzenle2.php"); // Eğer güncelleme sorgusu çalıştıysa ekle.php sayfasına yönlendiriyoruz.
		}
		else
		{
